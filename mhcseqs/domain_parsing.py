...
from .domain_grammar import (
    ...
    CLASS_I_ALPHA2_CYS1_OFFSET,
    CLASS_I_ALPHA2_DOMINANT_SEP,
    CLASS_I_ALPHA2_END_AFTER_CYS2,
    CLASS_I_FRAGMENT_MAX_LEN,
    CLASS_II_ALPHA_FRAGMENT_MAX_LEN,
    CLASS_II_ALPHA_GENE_PREFIXES,
    CLASS_II_ALPHA_GROOVE_END_BEFORE_IG_CYS,
    CLASS_II_ALPHA_IG_DOMINANT_SEP,
    CLASS_II_BETA_FRAGMENT_MAX_LEN,
    CLASS_II_BETA_GENE_PREFIXES,
    CLASS_II_BETA_GROOVE_END_BEFORE_BETA2_CYS,
    CLASS_II_BETA1_DOMINANT_SEP,
    CLASS_II_BETA2_DOMINANT_SEP,
    ...
    SP_CLEAVAGE_RESIDUES as _SP_CLEAVAGE_RESIDUES,
...
class ParseScaffold:
    ...
    def score_components(self, mature_start: int) -> ParseSubscores:
        ...
        if self.grammar.length_mode == "class_i_split":
            ...
            groove += (
                _soft_prior_score(groove_len, *_GROOVE_SEGMENT_PRIOR[self.grammar.groove_prior_key])
                if groove_len > 0
                else -30.0
            )
            if self.beta1_c1 > 0:
...
def _selected_sp_estimate(
    ...
    *,
    ...
    if use_early_shortcuts:
        ...
        if (
            features.sp_shortcut_kind == "exact_mature10"
            and features.sp_shortcut_state == "leaderless"
        ):
            return 0
...
def classify_domain_fold(
    seq: str, c1: int, c2: int
) -> DomainFoldAnnotation:
...
def score_cys_flanking_properties(
    seq: str, c1: int, c2: int
) -> tuple[float, float]:
...
def _record_candidate_key(record: AlleleRecord) -> tuple[float, ...]:
    ...
    domain_count = sum(
        int(bool(value))
        for value in (record.groove1, record.groove2, record.ig_domain, record.tail)
    )
    candidate_type = candidate.candidate_type
    type_bonus = 0.0
...
def _score_sp_estimate_zone_consistency(
    ...
    for cand_pos, cand_score in candidates:
        ...
        if delta > 0:
            distance *= _SP_LATE_DRIFT_SCALE
        candidate_score = rel * (
            (1.0 + 2.5 * confidence) - (distance / span) * (2.0 + confidence)
        )
...
def _build_parse_scaffold(
    ...
    *,
    ...
    if grammar.length_mode == "class_i_split":
        ...
        if secondary_c1 > 0 and secondary_c2 > 0:
            fixed_groove += (
                secondary_g_topology * weights.groove_secondary_g_topology
                + secondary_between * weights.groove_secondary_between
            )
            fixed_groove += (
                _soft_prior_score(
...
def _enumerate_mature_starts(
    ...
    *,
    ...
    _typical, _soft, hard = _GROOVE_PRIOR.get(
        scaffold.grammar.groove_prior_key, (90, (75, 100), (65, 115))
    )
    hlo, hhi = hard
...
def refine_signal_peptide(
    ...
    *,
    ...
    if groove_anchor:
        ...
        if pairs:
            annotated = list(features.pair_annotations) if features else [
                classify_cys_pair(seq, c1, c2) for c1, c2, _sep in pairs
            ]
...
def _class_i_fragment_result(
    *,
    ...
    if has_alpha2_pair:
        return _attach_parse_candidate(AlleleRecord(
            allele=allele,
            gene=gene,
            mhc_class="I",
            chain="alpha",
...
def _prepare_parse_inputs(
    ...
    *,
    ...
    if len(cleaned) < MIN_GROOVE_SOURCE_LEN:
        return (
            _attach_parse_candidate(AlleleRecord(
                allele=allele,
                gene=gene,
    ...
    annotated = list(features.pair_annotations) if features else [
        classify_cys_pair(cleaned, c1, c2) for c1, c2, _sep in pairs
    ]
    if features:
...
def _build_beta_secondary_resolver(
    ...
    beta1_ann = (
        max(
            annotated,
            key=lambda a: (a.groove_score + _cached_topology_support(seq, a.c1, a.c2, "g_domain", features))
            - (0.01 * a.c1),
        )
        if annotated
        else None
...
def _score_primary_contradictions(
    ...
    *,
    ...
    evidence: list[str] = []
    structural_mass = (
        max(0.0, scaffold.fixed_groove)
        + 0.5 * max(0.0, scaffold.fixed_ig)
        + 0.25 * max(0.0, scaffold.fixed_tail)
    )
...
def _should_expand_primary_competition(
    ...
    *,
    ...
    if features is not None:
        if (
            features.sp_estimate_confidence >= PRIMARY_PARSE_LOW_CONFIDENCE
...
def _build_primary_result(
    *,
    ...
    if not _domains_match_grammar(grammar, domains):
        ...
        ), seq=seq, h_region=selection.scaffold.h_region, sp_estimate=sp_estimate, features=features)
    candidate_type = (
        _full_candidate_type(grammar)
        if domains.ig_domain
        else _missing_support_candidate_type(grammar)
    )
...
def _build_class_i_alpha3_salvage_result(
    *,
    ...
    return _attach_parse_candidate(AlleleRecord(
        ...
        anchor_cys2=c2,
        flags=_flags_to_tuple(flags),
    ), seq=seq, h_region=(features.h_region if features is not None else None), sp_estimate=(features.sp_estimate if features is not None else None), features=features)
...
def decompose_class_i(
    ...
    *,
    ...
    if alpha3_ann is not None:
        if (
            best_record is None
            or best_record.status != "ok"
            or best_record.parse_candidate.support_state != "support_present"
        ):
            _append_record_candidate(
                candidates,
...
def decompose_class_ii_beta(
    ...
    *,
    ...
    missing_flags = (
        ("missing_support_anchor", "beta1_salvage_failed")
        if beta1_ann is not None
        else grammar.missing_no_anchor_flags
    )
    _append_record_candidate(
        candidates,
        _missing_groove_result(
...
def decompose_domains(
    ...
    *,
    ...
    if nc not in ("I", "II"):
        ...
        return _attach_parse_candidate(AlleleRecord(
            allele=allele, gene=gene, mhc_class=mhc_class or "",
            chain=str(chain or ""), sequence=cleaned_seq, seq_len=len(cleaned_seq),
            status="missing_groove",
    ...
    if result is None:
        result = AlleleRecord(
            allele=allele, gene=gene, mhc_class="II",
            chain=str(chain or ""), sequence=features.seq, seq_len=features.seq_len,
            status="missing_groove",
