...
from scripts import enrich_sp_ground_truth as enrichment
...
@pytest.mark.parametrize("accession", ["A0A8I3PCD6", "F6WA04", "Q6MG98", "Q6MGA0", "Q9TQA6"])
def test_stored_benchmark_enrichment_keeps_its_selection_policy(monkeypatch, source_rows, accession):
    def deny_network(*_args, **_kwargs):
        pytest.fail("Stored benchmark enrichment must be offline")

    monkeypatch.setattr("urllib.request.urlopen", deny_network)
    raw = next(row for row in enrichment._read_csv(enrichment.GT_RAW_CSV) if row["accession"] == accession)
    expected = next(row for row in enrichment._read_csv(enrichment.GT_ENRICHED_CSV) if row["accession"] == accession)
    fixture = Path(__file__).parent / "fixtures" / "mhc_protein_identity.json"
    taxonomy = json.loads(fixture.read_text())["taxonomy"]
    merged = enrichment._merge_row(
        raw,
        artifacts={accession: {**source_rows[accession], "_metadata_source": "uniprot_corpus_artifact"}},
        taxonomy=taxonomy,
        label_curation=eligibility.load_label_curation(),
    )
    for field in ("accession", "sequence", "sp_length", "mhc_class", "chain", "label_status", "species_category"):
        assert merged[field] == expected[field]
    assert not eligibility.resolve_mhc_protein_label(source_rows[accession], {}).eligible
...
def test_known_non_mhc_genes_override_mhc_words_in_protein_name(source_rows, accession):
    label = eligibility.resolve_mhc_protein_label(source_rows[accession], {})
...
def test_vague_names_do_not_exclude_exact_mhc_sequence_matches(source_rows, unknown, known):
    ...
    assert eligibility.resolve_mhc_protein_label(reference, {}).eligible
    label = eligibility.resolve_mhc_protein_label(candidate, {})
...
def test_missing_identity_evidence_stays_unresolved(name):
    label = eligibility.resolve_mhc_protein_label({"Protein names": name}, {})
    assert label.disposition == "retain_unresolved"
...
def test_each_gene_token_is_checked_with_its_source_species(monkeypatch):
    ...
    label = eligibility.resolve_mhc_protein_label({"Gene Names": "opaque;alias,CIITA", "Organism": "Mus musculus"}, {})
    assert label.disposition == "exclude_non_mhc"
...
def test_conflicting_mhc_and_helper_gene_metadata_stays_unresolved(source_rows, accession):
    ...
    label = eligibility.resolve_mhc_protein_label(row, {})
    assert label.disposition == "retain_unresolved"
    assert not label.eligible
    # Do not resolve contradictory synonyms by relying on their list order.
    reversed_genes = " ".join(reversed(row["Gene Names"].split()))
    assert eligibility.resolve_mhc_protein_label({**row, "Gene Names": reversed_genes}, {}) == label
...
def test_explicit_accession_curation_still_takes_precedence(source_rows, disposition):
    ...
    label = eligibility.resolve_mhc_protein_label(source_rows["P33076"], {"P33076": decision})
